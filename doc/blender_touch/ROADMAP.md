...
| P011 — interação com um dedo | Concluída | O [comportamento definido](P011_ONE_FINGER_INTERACTION.md) está no Window Manager. Builds completo e Lite passaram com Ninja. No Galaxy Book, toque/seleção, teclado e arrasto padrão foram validados; a seleção acompanhou o dedo e terminou ao soltá-lo. Touchpad e S Pen passaram na regressão. |
| P012 — validar interação com um dedo | Concluída | Testes físicos no Galaxy Book confirmaram toque e seleção, arrasto padrão sem operação presa e regressão de teclado, touchpad e S Pen. |
